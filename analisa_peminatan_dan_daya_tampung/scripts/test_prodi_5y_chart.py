...
for y in years:
    df_s1[f'Sisa_{y}'] = df_s1[f'Daya Tampung {y}'] - df_s1[f'Daftar Ulang {y}']
    df_s1[f'FR_{y}'] = (df_s1[f'Daftar Ulang {y}'] / df_s1[f'Daya Tampung {y}'] * 100.0).round(1)

df_s1['Total_Sisa_5Thn'] = sum(df_s1[f'Sisa_{y}'] for y in years)
df_s1['Total_DT_5Thn'] = sum(df_s1[f'Daya Tampung {y}'] for y in years)
df_s1['Total_DU_5Thn'] = sum(df_s1[f'Daftar Ulang {y}'] for y in years)
df_s1['Rata_FR_5Thn'] = (df_s1['Total_DU_5Thn'] / df_s1['Total_DT_5Thn'] * 100.0).round(1)
...
# Top 15 Akumulasi Kursi Kosong 5 Tahun
top15_sisa_5y = df_s1.sort_values('Total_Sisa_5Thn', ascending=True).tail(15).reset_index(drop=True)

# Exclude prodi baru yang belum ada 5 tahun penuh (seperti Bisnis Digital atau Teknik Perminyakan) untuk keadilan evaluasi kronis
df_established = df_s1[df_s1['Daya Tampung 2022'] > 0].copy()
bottom12_fr_5y = df_established.sort_values('Rata_FR_5Thn', ascending=False).tail(12).reset_index(drop=True)
top12_fr_5y = df_established.sort_values('Rata_FR_5Thn', ascending=True).tail(12).reset_index(drop=True)

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(27, 11.5), dpi=300, gridspec_kw={'width_ratios': [1.22, 1.18]})

# -------------------------------------------------------------
# Panel 1: Top 15 Akumulasi Kursi Kosong 5 Tahun (2022–2026)
# -------------------------------------------------------------
y_pos1 = np.arange(len(top15_sisa_5y))
colors_sisa = ['#991B1B' if fr < 65.0 else '#D97706' if fr < 80.0 else '#2563EB' for fr in top15_sisa_5y['Rata_FR_5Thn']]

bars1 = ax1.barh(y_pos1, top15_sisa_5y['Total_Sisa_5Thn'], color=colors_sisa, height=0.64, alpha=0.9, edgecolor='#0F172A', linewidth=1.0, zorder=2)

ax1.set_yticks(y_pos1)
ax1.set_yticklabels(top15_sisa_5y['Label_Prodi'], fontsize=10.5, fontweight='bold', color='#1E293B')
max_s1 = top15_sisa_5y['Total_Sisa_5Thn'].max()
ax1.set_xlim(0, max_s1 * 1.65)
ax1.set_ylim(-0.8, len(top15_sisa_5y) - 0.2)
ax1.set_xlabel('Akumulasi Kursi Kosong 5 Tahun (Bangku Terbuang 2022–2026)', fontsize=11, fontweight='bold', color='#0F172A', labelpad=10)
ax1.set_title('A. Akumulasi Kursi Kosong 5 Tahun per Program Studi (2022–2026)\n15 Prodi Ini Menyumbang 3,850 Kursi Kosong (43.4% dari Total Defisit 8,881 Kampus)', 
...
# -------------------------------------------------------------
# Panel 2: Polaritas Keterisian Rata-rata 5 Tahun (2022–2026)
# -------------------------------------------------------------
combined_fr_5y = pd.concat([bottom12_fr_5y, top12_fr_5y]).reset_index(drop=True)
y_pos2 = np.arange(len(combined_fr_5y))

colors_fr2 = ['#DC2626' if i < 12 else '#059669' for i in range(len(combined_fr_5y))]
bars2 = ax2.barh(y_pos2, combined_fr_5y['Rata_FR_5Thn'], color=colors_fr2, height=0.64, alpha=0.9, edgecolor='#0F172A', linewidth=1.0, zorder=2)
...
ax2.annotate('JURANG POLARISASI HISTORIS 5 TAHUN: Bintang Konsisten vs Penyakit Kronis',
...
ax2.set_yticklabels(combined_fr_5y['Label_Prodi'], fontsize=9.8, fontweight='bold', color='#1E293B')
ax2.set_xlim(0, 148)
ax2.set_ylim(-0.8, len(combined_fr_5y) - 0.2)
ax2.set_xlabel('Rata-rata Tingkat Keterisian Kuota 5 Tahun (Fill Rate %)', fontsize=11, fontweight='bold', color='#0F172A', labelpad=10)
ax2.set_title('B. Polaritas Historis 5 Tahun: 12 Bintang Konsisten vs 12 Krisis Kronis (2022–2026)\nMenghilangkan Bias 1 Tahun: Membuktikan Kinerja Reputasi Jangka Panjang Program Studi', 
...
out_file = os.path.join(chart_dir, "17_evaluasi_kinerja_program_studi_5_tahun_2022_2026.png")
plt.savefig(out_file, dpi=300)
plt.close()
print("Perfect Chart 17 generated successfully at:", out_file)
